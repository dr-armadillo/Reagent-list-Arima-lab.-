...

{
  ...
  let reagentSs;
  if (localStorage.getItem('reagentSs') === null) {
    reagentSs = [];
  } else {
    reagentSs = JSON.parse(localStorage.getItem('reagentSs'));
  }

  const savereagentSs = () => {
    localStorage.setItem('reagentSs', JSON.stringify(reagentSs));
  };

  const renderreagentS = (reagentS) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentS.isCompleted;
    input.addEventListener('change', () => {
      reagentSs.forEach((item) => {
        if (item.id === reagentS.id) {
    ...
    span.textContent = reagentS.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentSs = reagentSs.filter((item) => {
        return item.id !== reagentS.id;
      });
      savereagentSs();
    });
    ...
    document.querySelector('#reagentSs').appendChild(tr);
  }

  const renderreagentSs = () => {
    reagentSs.forEach((reagentS) => {
      renderreagentS(reagentS);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentS(reagentS);
    reagentSs.push(reagentS);
    console.table(reagentSs);
    savereagentSs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentSs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentSs = reagentSs.filter((reagentS) => {
      return reagentS.isCompleted === false;
    });
    savereagentSs();
    document.querySelectorAll('#reagentSs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentSs();
  });
