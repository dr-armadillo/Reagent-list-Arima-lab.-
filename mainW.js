...

{
  ...
  let reagentWs;
  if (localStorage.getItem('reagentWs') === null) {
    reagentWs = [];
  } else {
    reagentWs = JSON.parse(localStorage.getItem('reagentWs'));
  }

  const savereagentWs = () => {
    localStorage.setItem('reagentWs', JSON.stringify(reagentWs));
  };

  const renderreagentW = (reagentW) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentW.isCompleted;
    input.addEventListener('change', () => {
      reagentWs.forEach((item) => {
        if (item.id === reagentW.id) {
    ...
    span.textContent = reagentW.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentWs = reagentWs.filter((item) => {
        return item.id !== reagentW.id;
      });
      savereagentWs();
    });
    ...
    document.querySelector('#reagentWs').appendChild(tr);
  }

  const renderreagentWs = () => {
    reagentWs.forEach((reagentW) => {
      renderreagentW(reagentW);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentW(reagentW);
    reagentWs.push(reagentW);
    console.table(reagentWs);
    savereagentWs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentWs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentWs = reagentWs.filter((reagentW) => {
      return reagentW.isCompleted === false;
    });
    savereagentWs();
    document.querySelectorAll('#reagentWs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentWs();
  });
