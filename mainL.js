...

{
  ...
  let reagentLs;
  if (localStorage.getItem('reagentLs') === null) {
    reagentLs = [];
  } else {
    reagentLs = JSON.parse(localStorage.getItem('reagentLs'));
  }

  const savereagentLs = () => {
    localStorage.setItem('reagentLs', JSON.stringify(reagentLs));
  };

  const renderreagentL = (reagentL) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentL.isCompleted;
    input.addEventListener('change', () => {
      reagentLs.forEach((item) => {
        if (item.id === reagentL.id) {
    ...
    span.textContent = reagentL.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentLs = reagentLs.filter((item) => {
        return item.id !== reagentL.id;
      });
      savereagentLs();
    });
    ...
    document.querySelector('#reagentLs').appendChild(tr);
  }

  const renderreagentLs = () => {
    reagentLs.forEach((reagentL) => {
      renderreagentL(reagentL);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentL(reagentL);
    reagentLs.push(reagentL);
    console.table(reagentLs);
    savereagentLs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentLs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentLs = reagentLs.filter((reagentL) => {
      return reagentL.isCompleted === false;
    });
    savereagentLs();
    document.querySelectorAll('#reagentLs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentLs();
  });
