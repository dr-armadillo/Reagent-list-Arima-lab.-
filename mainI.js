...

{
  ...
  let reagentIs;
  if (localStorage.getItem('reagentIs') === null) {
    reagentIs = [];
  } else {
    reagentIs = JSON.parse(localStorage.getItem('reagentIs'));
  }

  const savereagentIs = () => {
    localStorage.setItem('reagentIs', JSON.stringify(reagentIs));
  };

  const renderreagentI = (reagentI) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentI.isCompleted;
    input.addEventListener('change', () => {
      reagentIs.forEach((item) => {
        if (item.id === reagentI.id) {
    ...
    span.textContent = reagentI.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentIs = reagentIs.filter((item) => {
        return item.id !== reagentI.id;
      });
      savereagentIs();
    });
    ...
    document.querySelector('#reagentIs').appendChild(tr);
  }

  const renderreagentIs = () => {
    reagentIs.forEach((reagentI) => {
      renderreagentI(reagentI);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentI(reagentI);
    reagentIs.push(reagentI);
    console.table(reagentIs);
    savereagentIs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentIs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentIs = reagentIs.filter((reagentI) => {
      return reagentI.isCompleted === false;
    });
    savereagentIs();
    document.querySelectorAll('#reagentIs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentIs();
  });
