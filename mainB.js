...

{
  ...
  let reagentBs;
  if (localStorage.getItem('reagentBs') === null) {
    reagentBs = [];
  } else {
    reagentBs = JSON.parse(localStorage.getItem('reagentBs'));
  }

  const savereagentBs = () => {
    localStorage.setItem('reagentBs', JSON.stringify(reagentBs));
  };

  const renderreagentB = (reagentB) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentB.isCompleted;
    input.addEventListener('change', () => {
      reagentBs.forEach((item) => {
        if (item.id === reagentB.id) {
    ...
    span.textContent = reagentB.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentBs = reagentBs.filter((item) => {
        return item.id !== reagentB.id;
      });
      savereagentBs();
    });
    ...
    document.querySelector('#reagentBs').appendChild(tr);
  }

  const renderreagentBs = () => {
    reagentBs.forEach((reagentB) => {
      renderreagentB(reagentB);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentB(reagentB);
    reagentBs.push(reagentB);
    console.table(reagentBs);
    savereagentBs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentBs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentBs = reagentBs.filter((reagentB) => {
      return reagentB.isCompleted === false;
    });
    savereagentBs();
    document.querySelectorAll('#reagentBs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentBs();
  });
