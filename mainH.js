...

{
  ...
  let reagentHs;
  if (localStorage.getItem('reagentHs') === null) {
    reagentHs = [];
  } else {
    reagentHs = JSON.parse(localStorage.getItem('reagentHs'));
  }

  const savereagentHs = () => {
    localStorage.setItem('reagentHs', JSON.stringify(reagentHs));
  };

  const renderreagentH = (reagentH) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentH.isCompleted;
    input.addEventListener('change', () => {
      reagentHs.forEach((item) => {
        if (item.id === reagentH.id) {
    ...
    span.textContent = reagentH.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentHs = reagentHs.filter((item) => {
        return item.id !== reagentH.id;
      });
      savereagentHs();
    });
    ...
    document.querySelector('#reagentHs').appendChild(tr);
  }

  const renderreagentHs = () => {
    reagentHs.forEach((reagentH) => {
      renderreagentH(reagentH);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentH(reagentH);
    reagentHs.push(reagentH);
    console.table(reagentHs);
    savereagentHs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentHs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentHs = reagentHs.filter((reagentH) => {
      return reagentH.isCompleted === false;
    });
    savereagentHs();
    document.querySelectorAll('#reagentHs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentHs();
  });
