...

{
  ...
  let reagentNs;
  if (localStorage.getItem('reagentNs') === null) {
    reagentNs = [];
  } else {
    reagentNs = JSON.parse(localStorage.getItem('reagentNs'));
  }

  const savereagentNs = () => {
    localStorage.setItem('reagentNs', JSON.stringify(reagentNs));
  };

  const renderreagentN = (reagentN) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentN.isCompleted;
    input.addEventListener('change', () => {
      reagentNs.forEach((item) => {
        if (item.id === reagentN.id) {
    ...
    span.textContent = reagentN.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentNs = reagentNs.filter((item) => {
        return item.id !== reagentN.id;
      });
      savereagentNs();
    });
    ...
    document.querySelector('#reagentNs').appendChild(tr);
  }

  const renderreagentNs = () => {
    reagentNs.forEach((reagentN) => {
      renderreagentN(reagentN);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentN(reagentN);
    reagentNs.push(reagentN);
    console.table(reagentNs);
    savereagentNs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentNs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentNs = reagentNs.filter((reagentN) => {
      return reagentN.isCompleted === false;
    });
    savereagentNs();
    document.querySelectorAll('#reagentNs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentNs();
  });
