...

{
  ...
  let reagentQs;
  if (localStorage.getItem('reagentQs') === null) {
    reagentQs = [];
  } else {
    reagentQs = JSON.parse(localStorage.getItem('reagentQs'));
  }

  const savereagentQs = () => {
    localStorage.setItem('reagentQs', JSON.stringify(reagentQs));
  };

  const renderreagentQ = (reagentQ) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentQ.isCompleted;
    input.addEventListener('change', () => {
      reagentQs.forEach((item) => {
        if (item.id === reagentQ.id) {
    ...
    span.textContent = reagentQ.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentQs = reagentQs.filter((item) => {
        return item.id !== reagentQ.id;
      });
      savereagentQs();
    });
    ...
    document.querySelector('#reagentQs').appendChild(tr);
  }

  const renderreagentQs = () => {
    reagentQs.forEach((reagentQ) => {
      renderreagentQ(reagentQ);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentQ(reagentQ);
    reagentQs.push(reagentQ);
    console.table(reagentQs);
    savereagentQs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentQs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentQs = reagentQs.filter((reagentQ) => {
      return reagentQ.isCompleted === false;
    });
    savereagentQs();
    document.querySelectorAll('#reagentQs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentQs();
  });
