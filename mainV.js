...

{
  ...
  let reagentVs;
  if (localStorage.getItem('reagentVs') === null) {
    reagentVs = [];
  } else {
    reagentVs = JSON.parse(localStorage.getItem('reagentVs'));
  }

  const savereagentVs = () => {
    localStorage.setItem('reagentVs', JSON.stringify(reagentVs));
  };

  const renderreagentV = (reagentV) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentV.isCompleted;
    input.addEventListener('change', () => {
      reagentVs.forEach((item) => {
        if (item.id === reagentV.id) {
    ...
    span.textContent = reagentV.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentVs = reagentVs.filter((item) => {
        return item.id !== reagentV.id;
      });
      savereagentVs();
    });
    ...
    document.querySelector('#reagentVs').appendChild(tr);
  }

  const renderreagentVs = () => {
    reagentVs.forEach((reagentV) => {
      renderreagentV(reagentV);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentV(reagentV);
    reagentVs.push(reagentV);
    console.table(reagentVs);
    savereagentVs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentVs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentVs = reagentVs.filter((reagentV) => {
      return reagentV.isCompleted === false;
    });
    savereagentVs();
    document.querySelectorAll('#reagentVs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentVs();
  });
