...

{
  ...
  let reagentKs;
  if (localStorage.getItem('reagentKs') === null) {
    reagentKs = [];
  } else {
    reagentKs = JSON.parse(localStorage.getItem('reagentKs'));
  }

  const savereagentKs = () => {
    localStorage.setItem('reagentKs', JSON.stringify(reagentKs));
  };

  const renderreagentK = (reagentK) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentK.isCompleted;
    input.addEventListener('change', () => {
      reagentKs.forEach((item) => {
        if (item.id === reagentK.id) {
    ...
    span.textContent = reagentK.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentKs = reagentKs.filter((item) => {
        return item.id !== reagentK.id;
      });
      savereagentKs();
    });
    ...
    document.querySelector('#reagentKs').appendChild(tr);
  }

  const renderreagentKs = () => {
    reagentKs.forEach((reagentK) => {
      renderreagentK(reagentK);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentK(reagentK);
    reagentKs.push(reagentK);
    console.table(reagentKs);
    savereagentKs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentKs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentKs = reagentKs.filter((reagentK) => {
      return reagentK.isCompleted === false;
    });
    savereagentKs();
    document.querySelectorAll('#reagentKs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentKs();
  });
