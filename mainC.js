...

{
  ...
  let reagentCs;
  if (localStorage.getItem('reagentCs') === null) {
    reagentCs = [];
  } else {
    reagentCs = JSON.parse(localStorage.getItem('reagentCs'));
  }

  const savereagentCs = () => {
    localStorage.setItem('reagentCs', JSON.stringify(reagentCs));
  };

  const renderreagentC = (reagentC) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentC.isCompleted;
    input.addEventListener('change', () => {
      reagentCs.forEach((item) => {
        if (item.id === reagentC.id) {
    ...
    span.textContent = reagentC.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentCs = reagentCs.filter((item) => {
        return item.id !== reagentC.id;
      });
      savereagentCs();
    });
    ...
    document.querySelector('#reagentCs').appendChild(tr);
  }

  const renderreagentCs = () => {
    reagentCs.forEach((reagentC) => {
      renderreagentC(reagentC);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentC(reagentC);
    reagentCs.push(reagentC);
    console.table(reagentCs);
    savereagentCs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentCs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentCs = reagentCs.filter((reagentC) => {
      return reagentC.isCompleted === false;
    });
    savereagentCs();
    document.querySelectorAll('#reagentCs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentCs();
  });
