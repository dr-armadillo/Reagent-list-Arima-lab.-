...

{
  ...
  let reagentGs;
  if (localStorage.getItem('reagentGs') === null) {
    reagentGs = [];
  } else {
    reagentGs = JSON.parse(localStorage.getItem('reagentGs'));
  }

  const savereagentGs = () => {
    localStorage.setItem('reagentGs', JSON.stringify(reagentGs));
  };

  const renderreagentG = (reagentG) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentG.isCompleted;
    input.addEventListener('change', () => {
      reagentGs.forEach((item) => {
        if (item.id === reagentG.id) {
    ...
    span.textContent = reagentG.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentGs = reagentGs.filter((item) => {
        return item.id !== reagentG.id;
      });
      savereagentGs();
    });
    ...
    document.querySelector('#reagentGs').appendChild(tr);
  }

  const renderreagentGs = () => {
    reagentGs.forEach((reagentG) => {
      renderreagentG(reagentG);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentG(reagentG);
    reagentGs.push(reagentG);
    console.table(reagentGs);
    savereagentGs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentGs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentGs = reagentGs.filter((reagentG) => {
      return reagentG.isCompleted === false;
    });
    savereagentGs();
    document.querySelectorAll('#reagentGs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentGs();
  });
