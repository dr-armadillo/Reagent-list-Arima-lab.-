...

{
  ...
  let reagentMs;
  if (localStorage.getItem('reagentMs') === null) {
    reagentMs = [];
  } else {
    reagentMs = JSON.parse(localStorage.getItem('reagentMs'));
  }

  const savereagentMs = () => {
    localStorage.setItem('reagentMs', JSON.stringify(reagentMs));
  };

  const renderreagentM = (reagentM) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentM.isCompleted;
    input.addEventListener('change', () => {
      reagentMs.forEach((item) => {
        if (item.id === reagentM.id) {
    ...
    span.textContent = reagentM.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentMs = reagentMs.filter((item) => {
        return item.id !== reagentM.id;
      });
      savereagentMs();
    });
    ...
    document.querySelector('#reagentMs').appendChild(tr);
  }

  const renderreagentMs = () => {
    reagentMs.forEach((reagentM) => {
      renderreagentM(reagentM);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentM(reagentM);
    reagentMs.push(reagentM);
    console.table(reagentMs);
    savereagentMs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentMs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentMs = reagentMs.filter((reagentM) => {
      return reagentM.isCompleted === false;
    });
    savereagentMs();
    document.querySelectorAll('#reagentMs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentMs();
  });
