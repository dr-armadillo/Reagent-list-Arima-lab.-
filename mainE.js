...

{
  ...
  let reagentEs;
  if (localStorage.getItem('reagentEs') === null) {
    reagentEs = [];
  } else {
    reagentEs = JSON.parse(localStorage.getItem('reagentEs'));
  }

  const savereagentEs = () => {
    localStorage.setItem('reagentEs', JSON.stringify(reagentEs));
  };

  const renderreagentE = (reagentE) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentE.isCompleted;
    input.addEventListener('change', () => {
      reagentEs.forEach((item) => {
        if (item.id === reagentE.id) {
    ...
    span.textContent = reagentE.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentEs = reagentEs.filter((item) => {
        return item.id !== reagentE.id;
      });
      savereagentEs();
    });
    ...
    document.querySelector('#reagentEs').appendChild(tr);
  }

  const renderreagentEs = () => {
    reagentEs.forEach((reagentE) => {
      renderreagentE(reagentE);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentE(reagentE);
    reagentEs.push(reagentE);
    console.table(reagentEs);
    savereagentEs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentEs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentEs = reagentEs.filter((reagentE) => {
      return reagentE.isCompleted === false;
    });
    savereagentEs();
    document.querySelectorAll('#reagentEs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentEs();
  });
