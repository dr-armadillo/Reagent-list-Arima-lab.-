...

{
  ...
  let reagentUs;
  if (localStorage.getItem('reagentUs') === null) {
    reagentUs = [];
  } else {
    reagentUs = JSON.parse(localStorage.getItem('reagentUs'));
  }

  const savereagentUs = () => {
    localStorage.setItem('reagentUs', JSON.stringify(reagentUs));
  };

  const renderreagentU = (reagentU) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentU.isCompleted;
    input.addEventListener('change', () => {
      reagentUs.forEach((item) => {
        if (item.id === reagentU.id) {
    ...
    span.textContent = reagentU.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentUs = reagentUs.filter((item) => {
        return item.id !== reagentU.id;
      });
      savereagentUs();
    });
    ...
    document.querySelector('#reagentUs').appendChild(tr);
  }

  const renderreagentUs = () => {
    reagentUs.forEach((reagentU) => {
      renderreagentU(reagentU);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentU(reagentU);
    reagentUs.push(reagentU);
    console.table(reagentUs);
    savereagentUs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentUs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentUs = reagentUs.filter((reagentU) => {
      return reagentU.isCompleted === false;
    });
    savereagentUs();
    document.querySelectorAll('#reagentUs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentUs();
  });
