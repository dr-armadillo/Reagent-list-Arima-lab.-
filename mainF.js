...

{
  ...
  let reagentFs;
  if (localStorage.getItem('reagentFs') === null) {
    reagentFs = [];
  } else {
    reagentFs = JSON.parse(localStorage.getItem('reagentFs'));
  }

  const savereagentFs = () => {
    localStorage.setItem('reagentFs', JSON.stringify(reagentFs));
  };

  const renderreagentF = (reagentF) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentF.isCompleted;
    input.addEventListener('change', () => {
      reagentFs.forEach((item) => {
        if (item.id === reagentF.id) {
    ...
    span.textContent = reagentF.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentFs = reagentFs.filter((item) => {
        return item.id !== reagentF.id;
      });
      savereagentFs();
    });
    ...
    document.querySelector('#reagentFs').appendChild(tr);
  }

  const renderreagentFs = () => {
    reagentFs.forEach((reagentF) => {
      renderreagentF(reagentF);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentF(reagentF);
    reagentFs.push(reagentF);
    console.table(reagentFs);
    savereagentFs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentFs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentFs = reagentFs.filter((reagentF) => {
      return reagentF.isCompleted === false;
    });
    savereagentFs();
    document.querySelectorAll('#reagentFs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentFs();
  });
