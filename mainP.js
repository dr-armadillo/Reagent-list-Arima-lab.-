...

{
  ...
  let reagentPs;
  if (localStorage.getItem('reagentPs') === null) {
    reagentPs = [];
  } else {
    reagentPs = JSON.parse(localStorage.getItem('reagentPs'));
  }

  const savereagentPs = () => {
    localStorage.setItem('reagentPs', JSON.stringify(reagentPs));
  };

  const renderreagentP = (reagentP) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentP.isCompleted;
    input.addEventListener('change', () => {
      reagentPs.forEach((item) => {
        if (item.id === reagentP.id) {
    ...
    span.textContent = reagentP.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentPs = reagentPs.filter((item) => {
        return item.id !== reagentP.id;
      });
      savereagentPs();
    });
    ...
    document.querySelector('#reagentPs').appendChild(tr);
  }

  const renderreagentPs = () => {
    reagentPs.forEach((reagentP) => {
      renderreagentP(reagentP);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentP(reagentP);
    reagentPs.push(reagentP);
    console.table(reagentPs);
    savereagentPs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentPs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentPs = reagentPs.filter((reagentP) => {
      return reagentP.isCompleted === false;
    });
    savereagentPs();
    document.querySelectorAll('#reagentPs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentPs();
  });
