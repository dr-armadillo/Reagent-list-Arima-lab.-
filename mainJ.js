...

{
  ...
  let reagentJs;
  if (localStorage.getItem('reagentJs') === null) {
    reagentJs = [];
  } else {
    reagentJs = JSON.parse(localStorage.getItem('reagentJs'));
  }

  const savereagentJs = () => {
    localStorage.setItem('reagentJs', JSON.stringify(reagentJs));
  };

  const renderreagentJ = (reagentJ) => {
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = reagentJ.isCompleted;
    input.addEventListener('change', () => {
      reagentJs.forEach((item) => {
        if (item.id === reagentJ.id) {
    ...
    span.textContent = reagentJ.english;
    ...
    button.addEventListener('click', () => {
      ...
      tr.remove();
      reagentJs = reagentJs.filter((item) => {
        return item.id !== reagentJ.id;
      });
      savereagentJs();
    });
    ...
    document.querySelector('#reagentJs').appendChild(tr);
  }

  const renderreagentJs = () => {
    reagentJs.forEach((reagentJ) => {
      renderreagentJ(reagentJ);
    });
  };
  ...
  document.querySelector('#add-form').addEventListener('submit', (e) => {
    ...
    renderreagentJ(reagentJ);
    reagentJs.push(reagentJ);
    console.table(reagentJs);
    savereagentJs();
  ...
  window.addEventListener('load', function () {
	  ...
	  document.querySelectorAll('#reagentJs th').forEach(elm => {
  ...
  document.querySelector('#purge').addEventListener('click', () => {
    ...
    reagentJs = reagentJs.filter((reagentJ) => {
      return reagentJ.isCompleted === false;
    });
    savereagentJs();
    document.querySelectorAll('#reagentJs tr').forEach((tr) => {
      tr.remove();
    });
    renderreagentJs();
  });
